		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				var html= '';
		            ...
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="businessService.images.image" '+multiple+'>';
		            html += '				</div>';
				    ...
				    html += '							<img class="upload_image" src="images/add_pic.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				var html = "";
				var imgWidth = parseInt(para.itemWidth.replace("px", ""))-15;
				var imgHeight = parseInt(para.itemHeight.replace("px", ""))-15;
				...
				// 图片上传的是图片还是其他类型文件
					...
					html += '		<div class="uploadImg" style="width:'+imgWidth+'px">';				
					/*html += '			<img id="uploadImage_'+file.index+'" class="upload_image" src="../' + e.target.result + '" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width);height:expression(this.height > '+imgHeight+' ? '+imgHeight+'px : this.height)" />'; */                   
					html += '			<img id="uploadImage_'+file.index+'" class="upload_image" src="' + e.target.result + '" style="width:100px;height:100px"/>';                                   
					html += '		</div>';
					html += '	</a>';
